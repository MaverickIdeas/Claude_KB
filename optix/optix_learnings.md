...
- **USB plugin bundle** at `C:\Users\TCarter\optix-plugin` (v1.1.0): marketplace `maverick-tools`, plugin `optix` → command `/optix:watch-inprogress` + both setup scripts. Install offline: copy folder to USB → `/plugin marketplace add <path>` → `/plugin install optix@maverick-tools` → `/reload-plugins`. (Repo also has the loose `/watch-inprogress`.)
